...
/* Read-only recording library. All filters work on the complete loaded catalog. */
const TEXT = {
  de: {
...
export function recordingMarkup(row, language, displayMode = "details") {
  ...
  if (displayMode === "rows") {
    ...
    return `<li class="recording-row"><details><summary><span class="row-title">${title}</span>${columns}</summary>${metadata}</details></li>`;
  }
  return `<li><h3>${title}</h3>${metadata}</li>`;
}
...
export class Enigma2RecordingsCard extends HTMLElement {
  ...
  setConfig(config) {
    ...
    this.config = {...config, display_mode: config.display_mode === "rows" ? "rows" : "details"}; this.identity = undefined; this.library.reset(); this.filters = {}; this.render();
  }
  set hass(hass) {
    this._hass = hass;
    ...
    if (identity !== this.identity) { this.identity = identity; this.library.reset(); this.filters = {}; }
    const language = lang(hass);
    if (language !== this.language || !this.shadowRoot.querySelector(".load")) { this.language = language; this.render(); }
    else this.update();
  }
  disconnectedCallback() { this.library.reset(); }
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `<ha-card><style>
      ...
      .recording-row dl{padding:4px 0 14px 22px}
      ...
      <p class="status" role="status" aria-live="polite"></p><ul aria-label="${text.title}"></ul></section></ha-card>`;
